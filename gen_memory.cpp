...
void matrix_transpose(int rows, int cols, int current_index, int* input, int* output) {
	...
	input[current_index + MLEN/2] = (rows << 16) | (cols);
	output[current_index + MLEN/2] = (rows << 16) | (cols);
	current_index++;
	...
	for (int i = 0; i<rows; i++) {
		for (int j = 0; j<cols; j++) {
			int val = rand();
			input[current_index + i * rows + j] = val;
			output[current_index + i * rows + j] = val;
			output[current_index + MLEN/2 + j * cols + i] = val;
		}
	}
...
int main() {
	...
	matrix_transpose(4, 4, current_index, input, output);
